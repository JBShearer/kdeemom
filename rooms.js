...
function paintKids(c){
  ...
  // Holly's room door — back wall left of poster
  D(c,130,155,55,115,P.dbrown);RR(c,133,158,49,109,4,"#4a2a10");
  c.fillStyle="rgba(0,0,0,0.4)";c.fillRect(134,159,47,107);
  c.fillStyle="#c0392b";c.font="bold 5px monospace";c.textAlign="center";c.fillText("DO NOT",157,185);c.fillText("ENTER",157,195);c.textAlign="left";
  c.fillStyle=P.gold;c.beginPath();c.arc(175,215,3,0,Math.PI*2);c.fill();
  // Greyson's room door — back wall right of poster
  D(c,260,140,55,110,P.dbrown);RR(c,263,143,49,104,4,"#4a2a10");
  c.fillStyle="rgba(20,10,5,0.3)";c.fillRect(264,144,47,102);
  c.fillStyle="#888";c.beginPath();c.arc(303,197,3,0,Math.PI*2);c.fill();
  c.fillStyle="rgba(255,215,0,0.5)";c.font="bold 5px monospace";c.textAlign="center";c.fillText("GREYSON",287,170);c.textAlign="left";
  // Mr. Rex
...
function paintGarage(c){
  ...
  // Audi
  RR(c,195,385,140,50,5,"#e74c3c");RR(c,208,372,100,16,4,"#c0392b");
...
function paintLaundry(c){
  ...
  // Downstairs steps — left floor corner, set into the floor properly
  var sx=8,sy=488;
  // Dark opening/pit behind stairs
  c.fillStyle="#0a0a0a";c.fillRect(sx,sy+20,80,50);
  c.fillStyle="#0a0a0a";c.beginPath();c.ellipse(sx+40,sy+22,30,8,0,0,Math.PI*2);c.fill();
  // Steps descending into darkness
  for(var st=0;st<4;st++){var sw=80-st*10,sxo=sx+st*5,syo=sy+st*8;D(c,sxo,syo,sw,9,st%2===0?"#654321":"#7a6040");}
  // Handrails
  D(c,sx+4,sy-8,3,34,"#8B6914");D(c,sx+76,sy-8,3,34,"#8B6914");
  c.strokeStyle="#8B6914";c.lineWidth=2;c.beginPath();c.moveTo(sx+4,sy-6);c.lineTo(sx+76,sy-6);c.stroke();
  // "BASEMENT" label
  c.fillStyle="rgba(255,215,0,0.5)";c.font="bold 5px monospace";c.textAlign="center";
  c.fillText("BASEMENT",sx+40,sy-10);c.textAlign="left";
}
...
function paintBasement(c){
  ...
  // Stairs going UP — left wall, ascending to laundry room
  var sx=0,sy=150;
  // Opening in left wall
  D(c,sx,sy,72,180,P.dbrown);
  RR(c,sx+2,sy+4,68,176,4,"#1a0a04");
  // Arch top
  c.fillStyle="#1a0a04";c.beginPath();c.arc(sx+36,sy+4,34,Math.PI,0);c.fill();
  // Steps ascending left to right (going up means far steps are higher)
  var stC=["#6a5030","#5a4020","#4a3010","#3a2000"];
  for(var us=0;us<4;us++){var uw=60-us*8,uy=sy+180-40-us*28;D(c,sx+4+us*4,uy,uw,10,stC[us]);}
  // Handrails
  c.strokeStyle="#8B6914";c.lineWidth=1.5;
  c.beginPath();c.moveTo(sx+8,sy+180-8);c.lineTo(sx+18,sy+20);c.stroke();
  c.beginPath();c.moveTo(sx+62,sy+180-8);c.lineTo(sx+56,sy+20);c.stroke();
  // "UPSTAIRS" label
  c.fillStyle="rgba(255,215,0,0.55)";c.font="bold 5px monospace";c.textAlign="center";
  c.fillText("UPSTAIRS",sx+36,sy+180+12);c.textAlign="left";
}
...
function paintGreyson(c){
  D(c,0,370,360,270,"#3a3020");
  // Wooden floor planks
  for(var i=0;i<360;i+=38){D(c,i,370,1,270,"rgba(0,0,0,0.08)");}
  // Warm sandy/desert wall tone
  var g=c.createLinearGradient(0,0,0,370);g.addColorStop(0,"#c8a870");g.addColorStop(1,"#b89a5a");c.fillStyle=g;c.fillRect(0,0,360,370);
  WF(c,370);

  // === SCIENCE CORNER (left wall) ===
  // Periodic table poster
  RR(c,10,45,80,65,3,"#fff");
  c.strokeStyle="#3498db";c.lineWidth=1;c.strokeRect(12,47,76,61);
  c.fillStyle="#3498db";c.font="bold 5px monospace";c.fillText("PERIODIC TABLE",13,56);
  // Mini element boxes
  var elCols=["#e74c3c","#3498db","#2ecc71","#FFD700","#9b59b6","#FF8C00"];
  for(var ei=0;ei<12;ei++){
    RR(c,13+(ei%6)*12,58+Math.floor(ei/6)*12,10,10,1,elCols[ei%6]);
    c.fillStyle="#fff";c.font="bold 4px monospace";c.fillText(["H","He","Li","Be","B","C","N","O","F","Ne","Na","Mg"][ei],15+(ei%6)*12,66+Math.floor(ei/6)*12);
  }

  // Chemistry set on left desk
  D(c,10,250,90,8,P.brown);D(c,15,258,3,80,P.dbrown);D(c,97,258,3,80,P.dbrown);
  // Beakers and flasks
  [[18,220,"#3498db","?"],[32,215,"#2ecc71","!"],[48,222,"#e74c3c","X"],[62,218,"#9b59b6","~"]].forEach(function(b){
    // Flask shape
    c.fillStyle=b[2];c.globalAlpha=0.5;
    c.beginPath();c.moveTo(b[0]+4,b[1]);c.lineTo(b[0]+4,b[1]+10);c.lineTo(b[0],b[1]+18);c.lineTo(b[0]+8,b[1]+18);c.lineTo(b[0]+4,b[1]+10);c.fill();
    c.globalAlpha=1;c.fillStyle=b[2];
    RR(c,b[0]+1,b[1]-6,6,8,2,b[2]);
    c.fillStyle="#fff";c.font="4px monospace";c.fillText(b[3],b[0]+2,b[1]+14);
  });
  // Bubbling tube
  c.strokeStyle="#00CED1";c.lineWidth=1.5;
  c.beginPath();c.moveTo(78,258);c.quadraticCurveTo(90,240,78,225);c.stroke();

  // Telescope (left corner, pointing at window)
  D(c,14,165,5,55,"#555");// tripod leg
  D(c,20,165,5,55,"#555");// tripod leg
  D(c,10,182,18,4,"#555");// brace
  // Telescope tube
  c.save();c.translate(22,162);c.rotate(-0.3);
  RR(c,-5,-4,50,9,4,"#888");RR(c,-5,-3,12,7,3,"#aaa");
  c.restore();
  c.fillStyle="#666";c.font="5px monospace";c.fillText("TELESCOPE",8,230);

  // === THE MARTIAN (center wall, framed poster) ===
  RR(c,128,40,104,110,4,"#8B6914");RR(c,132,44,96,102,3,"#c0392b");
  // Red planet surface
  var mg=c.createLinearGradient(132,44,132,146);mg.addColorStop(0,"#c0392b");mg.addColorStop(1,"#8B3010");c.fillStyle=mg;c.fillRect(132,44,96,102);
  // Planet curvature at bottom
  c.fillStyle="#a02800";c.beginPath();c.ellipse(180,146,48,12,0,0,Math.PI);c.fill();
  // Mars dust swirls
  c.strokeStyle="rgba(255,150,50,0.3)";c.lineWidth=2;
  c.beginPath();c.moveTo(138,90);c.quadraticCurveTo(160,70,185,95);c.stroke();
  c.beginPath();c.moveTo(165,108);c.quadraticCurveTo(185,95,210,115);c.stroke();
  // Martian figure (little green guy center of poster)
  c.fillStyle="#2ecc71";
  c.beginPath();c.ellipse(180,85,8,10,0,0,Math.PI*2);c.fill();// body
  c.beginPath();c.arc(180,74,7,0,Math.PI*2);c.fill();// head
  // Antennae
  c.strokeStyle="#2ecc71";c.lineWidth=1;
  c.beginPath();c.moveTo(177,68);c.lineTo(174,60);c.stroke();
  c.beginPath();c.moveTo(183,68);c.lineTo(186,60);c.stroke();
  c.fillStyle="#2ecc71";c.beginPath();c.arc(174,60,2,0,Math.PI*2);c.fill();
  c.beginPath();c.arc(186,60,2,0,Math.PI*2);c.fill();
  // Big eyes
  c.fillStyle="#fff";c.beginPath();c.ellipse(177,74,3,4,0,0,Math.PI*2);c.fill();
  c.beginPath();c.ellipse(183,74,3,4,0,0,Math.PI*2);c.fill();
  c.fillStyle="#000";c.beginPath();c.arc(177,74,1.5,0,Math.PI*2);c.fill();
  c.beginPath();c.arc(183,74,1.5,0,Math.PI*2);c.fill();
  // Tiny waving arm
  c.strokeStyle="#2ecc71";c.lineWidth=1.5;
  c.beginPath();c.moveTo(188,82);c.lineTo(196,76);c.stroke();
  c.fillStyle="#fff";c.font="bold 5px monospace";c.textAlign="center";
  c.fillText("THE MARTIAN",180,150);c.textAlign="left";

  // === WESTERN CORNER (right side) ===
  // Wanted poster on right wall
  RR(c,268,42,82,100,3,"#c9a56c");RR(c,272,46,74,92,2,"#b89a5a");
  c.fillStyle="#3d2b1f";c.font="bold 8px monospace";c.textAlign="center";
  c.fillText("WANTED",309,58);
  c.font="4px monospace";c.fillText("DEAD OR ALIVE",309,66);
  // Sheriff silhouette in wanted poster
  c.fillStyle="#3d2b1f";c.beginPath();c.arc(309,84,10,0,Math.PI*2);c.fill();// head
  c.fillStyle="#3d2b1f";c.beginPath();c.moveTo(298,94);c.lineTo(296,118);c.lineTo(322,118);c.lineTo(320,94);c.closePath();c.fill();// body
  // Cowboy hat on silhouette
  c.fillStyle="#1a0a00";
  c.beginPath();c.moveTo(298,84);c.lineTo(309,72);c.lineTo(320,84);c.closePath();c.fill();
  D(c,296,84,26,4,"#1a0a00");
  c.fillStyle="#c9a56c";c.font="bold 6px monospace";c.textAlign="center";
  c.fillText("$500",309,128);c.fillText("REWARD",309,136);c.textAlign="left";

  // Sheriff's badge (star) on shelf
  c.fillStyle="#FFD700";
  c.save();c.translate(292,188);
  c.beginPath();
  for(var pt=0;pt<5;pt++){
    var pa=pt*Math.PI*4/5-Math.PI/2;
    if(pt===0)c.moveTo(Math.cos(pa)*10,Math.sin(pa)*10);
    else c.lineTo(Math.cos(pa)*10,Math.sin(pa)*10);
    var pa2=pa+Math.PI*2/5;
    c.lineTo(Math.cos(pa2)*5,Math.sin(pa2)*5);
  }
  c.closePath();c.fill();
  c.fillStyle="#8B6914";c.font="bold 4px monospace";c.textAlign="center";c.fillText("SHERIFF",0,16);c.textAlign="left";
  c.restore();

  // Cowboy boots on floor (right side)
  [[258,455,"#3d2b1f"],[282,450,"#654321"]].forEach(function(b){
    // Boot shape
    RR(c,b[0],b[1],20,28,3,b[2]);
    RR(c,b[0]-2,b[1]+20,24,8,2,b[2]);// toe
    D(c,b[0]+7,b[1]-8,6,12,b[2]);// heel shaft
    // Spur
    c.strokeStyle="#FFD700";c.lineWidth=1;
    c.beginPath();c.arc(b[0]+22,b[1]+26,4,0,Math.PI*2);c.stroke();
  });

  // Lasso on wall peg
  c.strokeStyle="#8B6914";c.lineWidth=2;
  c.beginPath();c.arc(340,155,12,0,Math.PI*2);c.stroke();
  c.beginPath();c.moveTo(340,143);c.lineTo(340,130);c.stroke();
  D(c,337,126,6,4,"#5c3a1e");

  // Science journal / open book on desk
  RR(c,108,250,55,32,3,"#fff");
  c.strokeStyle="#3498db";c.lineWidth=0.8;
  for(var ln=0;ln<4;ln++){c.beginPath();c.moveTo(112,257+ln*6);c.lineTo(158,257+ln*6);c.stroke();}
  c.fillStyle="#e74c3c";c.font="5px monospace";c.fillText("HYPOTHESIS:",110,256);
  c.fillStyle="#333";c.font="4px monospace";c.fillText("Mars = Texas??",112,262);

  // Greyson's mixed-vibe desk chair (wooden, old west meets lab)
  RR(c,155,295,50,60,4,"#654321");
  D(c,160,260,40,40,"#c9a56c");// seat cushion
  D(c,160,258,40,4,"#b89a5a");// cushion top
}
...
// Room 0: Foyer
[{id:"fdoor",x:120,y:60,w:120,h:280,name:"Front Door",look:"Locked. You need keys.",open:"LOCKED."},{id:"shoes",x:245,y:420,w:80,h:60,name:"Shoe Pile",look:"Chaotic. None match.",push:"Cheerio rolls out."},{id:"coat",x:55,y:125,w:60,h:95,name:"Coat Rack",look:"Two coats, something alive.",use:"Gum wrapper and lint."},{id:"mirror",x:232,y:90,w:65,h:80,name:"Mirror",look:"Needs coffee. And keys.",talk:"'Have fun, don't die.'"},{id:"plant",x:228,y:320,w:70,h:85,name:"Suspicious Plant",look:"Thrives on chaos.",push:"A KEY falls out!",hasKey:true},{id:"fmat",x:115,y:468,w:130,h:50,name:"Welcome Mat",look:"WELCOME. Ironic.",push:"Dead bug and disappointment."},{id:"doorL",x:0,y:120,w:75,h:240,name:"Kitchen",open:"goto:1"},{id:"doorR",x:287,y:120,w:73,h:240,name:"Living Room",open:"goto:2"},{id:"jbdoor",x:0,y:430,w:30,h:90,name:"Jesus Bathroom",open:"goto:10"}],
// Room 1: Kitchen
...
// Room 6: Laundry
[{id:"washer",x:5,y:195,w:110,h:135,name:"Washer",look:"Running. Always.",open:"One sock."},{id:"dryer",x:115,y:195,w:110,h:135,name:"Dryer",look:"Lavender and regret.",open:"Three socks. None match."},{id:"laundry",x:220,y:375,w:135,h:105,name:"Mt. Washmore",look:"Sentient laundry.",push:"Missing homework."},{id:"iron",x:255,y:318,w:80,h:52,name:"Iron",look:"Last used 2021.",use:"Too much commitment."},{id:"shelf",x:228,y:35,w:125,h:65,name:"Shelf",look:"Almost empty bottles.",take:"Grabs tide pen."},{id:"lstairsdown",x:8,y:470,w:80,h:65,name:"Stairs Down",look:"Stairs down to the basement. Smells like projects and regret.",open:"goto:11"},{id:"ldoorB",x:0,y:260,w:14,h:200,name:"Kitchen",open:"goto:1"},{id:"mazedoor",x:346,y:380,w:14,h:150,name:"Strange Door",look:"Goes... down?",open:"goto:14"}],
// Room 7: Backyard
[{id:"tree",x:10,y:70,w:75,h:220,name:"Oak Tree",look:"Treehouse 'in progress' 2 years.",talk:"Talks to the tree."},{id:"cat",x:210,y:385,w:75,h:65,name:"Neighbor's Cat",look:"Judging everyone from next door. Again.",talk:"*slow blink* This cat owns all it surveys.",use:"Cat stares. Unmoving. Untouched."},{id:"hole",x:118,y:430,w:80,h:55,name:"The Hole",look:"Something dug this. Recently. Could be anything in there.",push:"Dig in!",use:"Dig in!"},{id:"bbq",x:278,y:355,w:72,h:75,name:"BBQ",look:"The BBQ sits in a minefield. The whole yard is covered in... evidence. The neighbor's cat is NOT responsible.",open:"K'Dee opens the BBQ lid. The smell triggers a chain reaction. Poop everywhere. Time to navigate."},{id:"fence",x:0,y:210,w:360,h:65,name:"Fence",look:"Keeps the cat out. Theoretically. It doesn't."},{id:"bdoorB",x:0,y:310,w:14,h:200,name:"Garage",open:"goto:5"},{id:"shed",x:88,y:325,w:95,h:80,name:"Shed",open:"goto:17"},{id:"garden",x:255,y:470,w:100,h:75,name:"Garden",open:"goto:18"}],
// Room 8: Attic
[{id:"xmas",x:5,y:300,w:62,h:68,name:"Xmas Box",look:"Tinsel entity.",open:"Tangled lights."},{id:"myst",x:72,y:292,w:78,h:75,name:"Mystery Box",look:"Nobody remembers this.",open:"Photo albums! VHS: DO NOT WATCH."},{id:"mirror2",x:118,y:140,w:80,h:105,name:"Old Mirror",look:"Spookier K'Dee.",talk:"Mirror-K'Dee winks."},{id:"trunk",x:242,y:318,w:110,h:60,name:"Trunk",look:"Locked. Treasure or taxes.",open:"Locked!",use:"Wrench opens it: old curtains."},{id:"crystal",x:298,y:278,w:55,h:38,name:"Crystal Ball",look:"Halloween 2022.",talk:"'Check the bedroom, dummy.'"},{id:"adoorR",x:346,y:370,w:14,h:150,name:"Bedroom",open:"goto:9"},{id:"gymdoor",x:188,y:352,w:75,h:55,name:"Gym",open:"goto:12"}],
// Room 9: Master Bedroom
[{id:"bed",x:10,y:248,w:212,h:110,name:"Bed",look:"Made this morning. Clothes pile using it as a base camp.",push:"More clothes. Not what you need."},{id:"clothespile",x:30,y:455,w:120,h:75,name:"Clothes Mountain",look:"Mt. Washmore's bedroom cousin. Phone is definitely in here.",push:"K'Dee digs in. The pile FIGHTS BACK. Clothes are flying!",take:"K'Dee reaches in. An avalanche starts. Stack them or be buried!"},{id:"nightstand",x:226,y:292,w:72,h:80,name:"Nightstand",look:"Book page 12 for 6 months.",open:"Charger #3, melatonin, and a KEY!",hasKey:true},{id:"vanity",x:228,y:200,w:120,h:75,name:"Vanity",look:"Lipstick, mascara, dry shampoo.",use:"Hair ties, bobby pins."},{id:"vmirror",x:250,y:118,w:88,h:78,name:"Mirror",look:"Survived another morning.",talk:"'Have fun, don't die.'"},{id:"window",x:88,y:20,w:130,h:115,name:"Window",look:"Neighbor's cat is staring in. Judging.",open:"'MOM!' from downstairs."},{id:"closet",x:300,y:82,w:60,h:115,name:"Closet",look:"Overflow situation. Two more piles fell out.",open:"An avalanche of shirts and regret."},{id:"flowers",x:228,y:168,w:58,h:28,name:"Flowers",look:"Self-care purchase.",take:"Smells nice."},{id:"bdoorL",x:0,y:160,w:14,h:210,name:"Attic",open:"goto:8"},{id:"forestdoor",x:346,y:270,w:14,h:200,name:"Forest's Den",open:"goto:21"},{id:"stairsDown",x:20,y:430,w:52,h:110,name:"Downstairs",open:"goto:2"}],
...
// Room 11: Basement
[{id:"penta",x:120,y:400,w:120,h:115,name:"Pentagram",look:"NOT home decor.",use:"Flames flare! Then nothing.",push:"Chalk smudges."},{id:"necro",x:28,y:415,w:75,h:65,name:"Necronomicon",look:"It whispers.",take:"'...return me.'",quest:"necronomicon",talk:"'Keys are not here, mortal.'"},{id:"skull",x:272,y:415,w:60,h:60,name:"Skull",look:"Real or prop?",talk:"'Alas, poor Yorick.'"},{id:"chains",x:5,y:70,w:55,h:150,name:"Chains",look:"Rusty. HISTORY.",use:"Echo: 10/10."},{id:"bstairsU",x:0,y:150,w:72,h:180,name:"Stairs Up",open:"goto:6"}],
// Room 12: Gym
[{id:"bench",x:18,y:258,w:140,h:85,name:"Bench Press",look:"Gains.",use:"Half a rep."},{id:"dumbbells",x:148,y:405,w:80,h:55,name:"Dumbbells",look:"Heavy.",take:"Almost lifts one."},{id:"gymmirror",x:82,y:118,w:192,h:125,name:"Mirror",look:"Looking STRONG.",talk:"'Find keys AND look great.'"},{id:"poster",x:268,y:95,w:85,h:70,name:"Poster",look:"NO PAIN NO GAIN."},{id:"pullup",x:250,y:88,w:100,h:70,name:"Pull-Up Bar",use:"3 seconds. Record."},{id:"mat",x:115,y:385,w:120,h:55,name:"Yoga Mat",use:"Namaste."},{id:"gymdoorB",x:0,y:370,w:14,h:150,name:"Attic",open:"goto:8"}],
// Room 13: Pantry
[{id:"shelves",x:8,y:15,w:344,h:350,name:"Shelves",look:"Expired 2022.",open:"Beans, mystery powder."},{id:"rice",x:255,y:308,w:100,h:72,name:"Rice Bag",look:"50 pounds.",push:"A map drawing!",take:"Too heavy."},{id:"pdoorB",x:0,y:270,w:14,h:200,name:"Kitchen",open:"goto:1"}],
// Room 14: Laundry Maze B1
[{id:"breakers",x:118,y:38,w:120,h:100,name:"Breakers",look:"Half tripped.",use:"Lights flicker! K'Dee jumps. Something scurries."},{id:"washers",x:5,y:185,w:350,h:135,name:"Machines",look:"EIGHT. WHY.",open:"One sock."},{id:"lostSock",x:15,y:355,w:60,h:65,name:"Lost Sock",look:"One sock. Red 'LOST' label. It's been alone for years.",take:"K'Dee picks it up. It's still warm somehow. From the dryer. Probably.",quest:"lostSock"},{id:"drip",x:80,y:352,w:78,h:65,name:"Pipe Drip",look:"Steady drip. Puddle forming.",push:"K'Dee moves the puddle mat. A 2003 quarter gleams underneath.",quest:"coin"},{id:"maze1R",x:346,y:270,w:14,h:200,name:"Deeper",open:"goto:15"},{id:"maze1L",x:0,y:270,w:14,h:200,name:"Laundry",open:"goto:6"}],
// Room 15: Laundry Maze B2
[{id:"sign",x:45,y:60,w:270,h:62,name:"Sign",look:"YOU ARE HERE... OR ARE YOU?"},{id:"morewash",x:5,y:215,w:248,h:110,name:"More Machines",look:"Laundry DIMENSION.",open:"47 socks. 0 matches."},{id:"sockpile",x:22,y:372,w:95,h:78,name:"Sock Pile",look:"Approximately 47 socks. All unmatched. One is still warm.",use:"Sock Sort!",push:"K'Dee counts them. 47 socks, 94 possibilities, 0 pairs. A tragedy."},{id:"mysteryMachine",x:258,y:210,w:80,h:115,name:"Mystery Machine",look:"This one hums differently. A faint blue glow inside the drum.",open:"K'Dee peers in. The drum shows... somewhere else. It smells like possibility. And dryer sheets."},{id:"maze2R",x:346,y:270,w:14,h:200,name:"Deeper",open:"goto:16"},{id:"maze2L",x:0,y:270,w:14,h:200,name:"Back",open:"goto:14"}],
// Room 16: Laundry Maze B3
[{id:"exit",x:105,y:5,w:150,h:55,name:"EXIT",look:"Can you trust it?"},{id:"lint",x:228,y:242,w:112,h:112,name:"Lint Monster",look:"Dryer lint with EYES. Yarn tendrils. It has LIVED here.",talk:"*rustles menacingly* ...then tilts head.",push:"Reforms. ALWAYS.",use:"Dryer sheet: it PURRS. Then dissolves. It smells like cinnamon and old tumble-dry. A key falls from the lint."},{id:"secretDrawer",x:238,y:330,w:90,h:45,name:"Hidden Drawer",look:"A tiny drawer behind where the Lint Monster sat. Suspicious.",open:"K'Dee pulls it. A slightly smushed banana inside. The Lint Monster was SAVING it."},{id:"escape",x:306,y:145,w:45,h:155,name:"Escape Door",open:"goto:6"},{id:"maze3L",x:0,y:270,w:14,h:200,name:"Back",open:"goto:15"}],
// Room 17: Shed
[{id:"mower",x:12,y:288,w:110,h:80,name:"Mower",look:"Grass was waist-high.",use:"Pull cord. Nothing."},{id:"tools2",x:130,y:58,w:60,h:280,name:"Tools",look:"Rakes, shovels.",take:"Takes shovel.",quest:"shovel"},{id:"tarp",x:185,y:200,w:140,h:105,name:"Tarp",look:"Something BIG.",push:"Holiday decorations."},{id:"pots",x:240,y:375,w:75,h:65,name:"Pots",look:"Dead plants.",push:"Dirt and regret."},{id:"sheddoorB",x:0,y:370,w:14,h:150,name:"Backyard",open:"goto:7"}],
// Room 18: Garden
[{id:"beds",x:10,y:195,w:350,h:100,name:"Garden Beds",look:"Tomatoes, herbs.",take:"Herbs for dinner."},{id:"sunflowers",x:222,y:210,w:132,h:100,name:"Sunflowers",look:"Majestic.",talk:"They lean toward her."},{id:"gnome",x:285,y:385,w:68,h:72,name:"Gnome",look:"Creepy smile. Watching.",talk:"'You know where the keys are.'",push:"Note: CHECK THE COUCH. In blood. Probably ketchup.",quest:"gnomeNote"},{id:"gardendoorB",x:0,y:200,w:14,h:300,name:"Backyard",open:"goto:7"}],
// Room 19: Greyson's Room
[{id:"gscience",x:5,y:30,w:118,h:270,name:"Chemistry Corner",look:"Flasks bubbling. Something is clearly a hypothesis gone wrong.",use:"Accidentally turns solution green. It smells like ranch.",talk:"The telescope is pointed at Mars. Ambitious."},{id:"gmartian",x:118,y:30,w:120,h:128,name:"The Martian Poster",look:"A framed poster of a little green guy waving on a red planet. Greyson drew it himself.",talk:"The Martian appears to wink. This room is a lot."},{id:"gwanted",x:252,y:30,w:100,h:120,name:"Wanted Poster",look:"WANTED: DEAD OR ALIVE. $500 reward. Greyson drew this too. The resemblance to no one is uncanny.",push:"The poster flutters. A star badge falls out."},{id:"gsheriff",x:256,y:132,w:72,h:80,name:"Sheriff Badge",look:"A genuine gold star. Greyson takes his western phase very seriously.",take:"Takes the badge. Feels surprisingly official."},{id:"gboots",x:238,y:428,w:88,h:65,name:"Cowboy Boots",look:"Real spurs. Greyson is committed to the bit.",push:"They clink. Something is in one — a folded note: CHECK GARAGE."},{id:"gjournal",x:90,y:232,w:88,h:65,name:"Science Journal",look:"'HYPOTHESIS: Mars = Texas??' Several pages of evidence. Some of it is maps.",open:"The journal falls open to: 'If I can survive Mars I can survive Mom.'"},{id:"gcookies",x:90,y:372,w:82,h:62,name:"Cookie Stash",look:"A zip-lock bag of peanut butter cookies hidden behind the telescope. Classic.",take:"K'Dee confiscates a cookie. Science can't stop her.",quest:"cookie"},{id:"groomdoorB",x:0,y:370,w:14,h:150,name:"Kids' Room",open:"goto:3"},{id:"groomdoorR",x:346,y:270,w:14,h:200,name:"Gwyneth's Room",open:"goto:20"}],
// Room 20: Gwyneth's Room
[{id:"gwbed",x:12,y:252,w:185,h:105,name:"Gwyneth's Bed",look:"Books on both pillows. She fell asleep reading. Again.",open:"'Reading is my superpower.' A book falls out. It was open to the last page."},{id:"hgvanity",x:232,y:130,w:120,h:95,name:"Vanity",look:"Lip gloss, hair dye, blue accessories everywhere.",use:"Borrows some blue eyeliner. Shhh."},{id:"fairy",x:0,y:55,w:360,h:30,name:"Fairy Lights",look:"Blue and teal. Magical.",use:"Disco mode activated."},{id:"stuffed",x:128,y:390,w:72,h:65,name:"Blue Bear",look:"Gwyneth's companion since age 4. Judges no one.",talk:"Mr. Blue-Bear has seen some things. He's at peace."},{id:"art",x:20,y:70,w:92,h:75,name:"Fashion Art",look:"Fashion Week 2025. K'Dee nods approvingly.",talk:"Gwyneth wants to be a designer. Obviously."},{id:"books",x:178,y:362,w:75,h:72,name:"Book Pile",look:"At least 40 books. All read multiple times.",open:"Fantasy, fashion history, and one suspiciously thick manga."},{id:"sewing",x:258,y:378,w:85,h:65,name:"Sewing Machine",look:"Mid-project. Blue fabric everywhere.",use:"One needle stuck in the fabric. K'Dee leaves it alone."},{id:"gwyndoor2",x:346,y:100,w:14,h:120,name:"Other Side",look:"A door to the other side of Gwyneth's room. She's sleeping right in the way.",use:"Sneak past!"},{id:"hgroomdoorB",x:0,y:370,w:14,h:150,name:"Greyson's Room",open:"goto:19"}],
// Room 21: Forest's Den
[{id:"bubble",x:95,y:320,w:175,h:72,name:"Gaming Bubble",look:"A sealed dome around his gaming setup. This is where he lives now.",talk:"*typing through glass* '...what.'"},{id:"snacks",x:65,y:395,w:100,h:70,name:"Snack Pile",look:"Six empty energy drink cans and 12 wrappers. He's been here a while.",take:"Confiscated snacks."},{id:"signs",x:5,y:62,w:80,h:95,name:"Door Signs",look:"QUARANTINE. GO AWAY. LOADING... DO NOT DISTURB — I MEAN IT. Biohazard symbol.",talk:"K'Dee knocks. A long pause. 'I SAID LOADING.'"},{id:"gpc2",x:88,y:88,w:242,h:228,name:"Gaming Setup",look:"Bigger than Greyson's. LED everywhere. He will NOT let you touch it.",use:"K'Dee accidentally moves a cable. Forest's reaction is INSTANT."},{id:"froomdoorB",x:0,y:270,w:14,h:200,name:"Bedroom",open:"goto:9"}]
];}
